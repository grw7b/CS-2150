...
mergeSort:
	...
	mov rcx, rdx 		 ; putting last index into rcx for merge
...
sort:	
	...
	cmp rsi, rdx		; if left index. < right index
	jge stopSorting		; if not, then return


	push rsi
	push rdx

	add rdx, rsi 		; m = right + left
	shr rdx, 1 			; m = (right + left)/2


	call sort 			; sorting first half


	inc rdx 			; m = m + 1

	mov rsi, rdx  		; start = m + 1  set last index back to original value 

	pop rdx

	call sort			; sort second half
	...
	call merge 			; merge halves of array
