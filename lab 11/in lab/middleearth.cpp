 ...
 * @param xsize x-axis
 * @param ysize y-axis
 * @param num_cities number of cities in middle-earth
 * @param seed randomly generated
 *
 */
...
MiddleEarth::MiddleEarth(int xsize, int ysize, int num_cities, int seed) {
    ...
    gen.seed(seed == -1 ? time(NULL) : seed);
...
/**
 * @brief Middle-Earth printer
 * Prints out info on the created 'world'
 * including names, number of cities, and positions
 *
 */
...
/**
 * @brief Middle-Earth print table
 * Prints a tab-separated table of the distances,
 * which can be loaded into Excel or similar
 *
 */
...
/**
 * @brief Middle-Earth get distance
 * This method returns the distance between the two passed cities.
 * 
 * @param city1
 * @param city2
 *
 * @return returns distance between two cities
 */
...
/**
 * @brief Middle-Earth get itinerary
 * 
 * Returns the list of cities to travel to.
 * The first city is the original start point as well as the end point.
 * The number of cities passed in does not include this start/end point
 * (so there will be length+1 entries in the returned vector).
 * 
 * @param length - number of all the cities except for start/end point
 * @return returns a vector containing all the citites 
 *
 */
